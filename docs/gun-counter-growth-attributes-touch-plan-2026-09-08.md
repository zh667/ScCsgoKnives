...
本节取代原五级经验方案及后续聊天中的“100杀满级”建议。用户本轮确认10级、每级100击杀、满级伤害+50%、耐久+50%、电击枪10秒→5秒，并保留满级娱乐向毕业效果。容量+50%沿用上一轮建议。电击枪基础攻击力27→54是针对“大幅提高”的具体设计建议，须实测，不伪称用户指定了54。**0.39.1尚未实现计数器或成长，本轮仅改规划。**
...
- **电击枪额外加强建议：**当前实际近距攻击力27，建议基础改为54（不安装计数器也生效），安装Lv0仍54，Lv10按同一+50%规则为81。不是套用CS2参考Damage500，也不增爆头倍率／眩晕时长。充能从10秒降到5秒后，满级相对当前版理论持续伤害由2.7攻击力/秒提升到16.2攻击力/秒，约6倍；这是强度预警，不是承诺实际扣血／击杀时间。基础54为设计初值，用户尚未指定这个绝对数值，实测可调整但保持明显加强方向。普通35枪基础表仍作为当前版本事实，实施时只覆盖电击枪这一明确例外，不能把历史表悄悄改成“已经加强”。
...
| Zeus | 建议基础54／54 → 满级81／81 | 1→1 | 100→150 | 建议4.575格（有限近距例外） | 10→5秒 |
...
- 成长伤害、容量、耐久均最大+50%；电击枪按基础54建议与满级81检查，5秒充能不能无限放电、不能获得爆头倍增；普通枪不被电击枪基础伤害覆盖误改。
