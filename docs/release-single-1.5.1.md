...
| 文件 | 字节 | 十进制MB |
|---|---:|---:|
| `output/[API1.9]CS武器1.5.1-作者ZH667-全量总包.scmod` | 491,121,204 | 491.12 |
| `output/[API1.9]CS武器1.5.1-作者ZH667-512轻量总包.scmod` | 88,433,762 | **88.43** |

相对原94,263,117字节轻量ZIP，总包减少 **5,829,357字节（5.83 MB / 6.18%）**。在既有512轻量资源上没有进一步降低画质、减少模型、删除声音或裁剪动画。

全量 SHA-256：`ab5f7bc7b9a11a6b1066766f599b2637e5b998355f04c47419fad3fc1b9db9a2`

轻量 SHA-256：`1f7d04b54b5957e5f433dda27fbc42d22747ff492db0c43c60ab4918238ea072`

## 这次进世界报错的原因与修复

上一版1.5.1总包的 `Assets/ScCsgoResources.xml` 使用了 `encoding='utf8'`。桌面.NET XML解析器会接受这个写法，但Survivalcraft原生XML读取器只接受标准名称 `utf-8`，所以模组进入世界时主动报“资源前置包缺失或不匹配：System does not support 'utf8' encoding”。这不是世界损坏，也不是主包/轻量资源缺失。

已将两版总包的XML声明改为标准 `encoding="utf-8"` 并重新生成。冷启动资源门控、世界枪械存档保护、原生XML读取器和两版完整PackageCheck均通过。请使用本页列出的新哈希文件替换上一版1.5.1总包。
...
- 实际总包主DLL回归：全量13,621项、轻量14,334项通过（含新增原生 XML 编码与资源清单检查）。
