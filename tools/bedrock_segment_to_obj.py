#!/usr/bin/env python3
"""Export one animated Bedrock segment using a common root normalization."""
...
def main():
    ...
    output_selected = set()
    pending = [args.segment]
    while pending:
        name = pending.pop()
        if name in output_selected:
            continue
        output_selected.add(name)
        pending.extend(b["name"] for b in bones.values() if b.get("parent") == name)
    ...
    for bone_name in selected:
        ...
        for cube in bone.get("cubes", []):
            ...
            for face_name, indices in face_corners.items():
                ...
                if bone_name in output_selected:
    ...
    # Drop unreferenced geometry that was retained only to calculate the common
    # normalization bounds, and compact OBJ indices for a small runtime asset.
    used_vertices = sorted({corner[0] for face in faces for corner in face})
    used_texcoords = sorted({corner[1] for face in faces for corner in face})
    used_normals = sorted({corner[2] for face in faces for corner in face})
    ...
    vertices = [vertices[index - 1] for index in used_vertices]
    texcoords = [texcoords[index - 1] for index in used_texcoords]
    normals = [normals[index - 1] for index in used_normals]
    faces = [[(vertex_map[vi], texcoord_map[ti], normal_map[ni]) for vi, ti, ni in face] for face in faces]

    if args.metadata:
        bone = bones[args.segment]
        point = [0.0, 0.0, 0.0]
        chain = []
        cursor = bone
        while cursor:
            ...
            cursor = bones.get(cursor.get("parent"))
        for ancestor in chain:
            point = transform_part(point, bone_position(ancestor, bones), ancestor.get("rotation", [0, 0, 0]))
        pivot = [(point[i] - center[i]) * scale for i in range(3)]
        args.metadata.write_text(json.dumps({"bone": args.segment, "pivot": pivot}, indent=2), encoding="utf-8")
