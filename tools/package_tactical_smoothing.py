"""Stage/publish the full-only 1.3.0 animation and loading optimization. Change one DLL only."""
import argparse
import hashlib
import json
import shutil
...
STAGE = ROOT / '.tmp/crowd-smooth-20260926'
NAME = '[API1.9]CS武器1.3.0-全量包.scmod'
SOURCE_SHA = '0c4f2fddc79b56b90a701e892fe9e52195bb02cbecfc1b2ee67aeb1cc2926929'
...
def stage():
    ...
    with zipfile.ZipFile(source) as old:
        ...
        with zipfile.ZipFile(target) as new:
            assert new.testzip() is None and set(new.namelist()) == set(old.namelist())
            ...
            report = dict(path=NAME, revision='crowd-smooth-20260926', sourceSha256=SOURCE_SHA,
                sha256=sha(target.read_bytes()), bytes=target.stat().st_size, changedEntries=['ScCsgoTactical.dll'],
                unchangedCompressedEntries=len(entries)-1,
...
def publish():
    ...
    for tool in ['ActorSamplingCheck','NpcWeaponCheck','TacticalPerformanceCheck','AppearanceCheck','TacticalRenderCheck']:
    ...
    sampling=read(STAGE/'sampling/checks.json');weapons=read(STAGE/'weapons/checks.json')
    for label in ['sampling','weapons']:
        assert read(STAGE/label/'checks.json')['failed']==0
        assert (STAGE/label/'checks.json').stat().st_mtime_ns>=candidate.stat().st_mtime_ns
    source=ROOT/'output'/NAME
    assert sha(source.read_bytes()) == SOURCE_SHA
    ...
    report.update(validation=validation,probe=probe,samplingChecks=len(sampling["checks"]),samplingMeasurements=sampling["measurements"],weaponChecks=len(weapons["checks"]),appearanceChecks=len(appearance['checks']),nativeActorFrames=len(actors),
        scope='Windows isolated native fixtures; no Android, actual player world, or all-installed-mod combination acceptance')
    (ROOT/'docs/release-crowd-smoothing-1.3.0-2026-09-26-evidence.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n','utf-8')
