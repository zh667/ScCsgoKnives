...
def main():
    ...
    for profile,edition,index in [('1.0.0','Full',0),('1.2.0','Full',1),('latest','Full',2),('latest','Lite',2)]:
        ...
        with zipfile.ZipFile(p) as z:
            assert sha(z.read('ScCsgoKnives.dll'))==matrix['modules'][index]['sha256']==report['coreSha256']
            assert json.loads(z.read('Integrations/CompatibilityFamily.json'))['backup_policy']=='manual'
        packages.append(report)
    for key in ['100','120','160','160-lite']:
    ...
    result={'family':1,'protocol':1,'layout':5,'schema':6,'rules':7,'base_commit':'6aade7b',
            'backup_policy':'manual','build_revision':'2026-09-25-manual-backup',
