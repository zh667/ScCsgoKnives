"""Release 1.5.3 with the verified, unchanged 1.5.1 resource streams and a tested core DLL."""
...
def main():
    ...
    check=json.loads((ROOT/'.tmp/balance-check-153.json').read_text('utf8'))
    ...
    with zipfile.ZipFile(source) as z:
        ...
        metadata=json.loads(z.read('modinfo.json'));metadata['Version']='1.5.3'
        metadata['Description']='CS武器1.5.3整合总包：修正1.2.0旧世界因局部枪械冲突无法进入的问题；验证完整备份，正常枪保留状态，异常枪保留待恢复且旧编号不再分配。全部枪械Lv0保持CS2基准射速。含完整战术同伴、敌对小队、拆弹与内置外观。全量与轻量二选一，停用旧独立CS主包、资源包和战术拓展。玩家CT/T外观另需NekoMeko Model1.1及Neorxna1.4。'
        add('modinfo.json',(json.dumps(metadata,ensure_ascii=False,indent=2)+'\n').encode('utf8'))
        core=json.loads(z.read('Integrations/ScCsgoKnives.modinfo.json'));core['Version']='1.5.3'
        add('Integrations/ScCsgoKnives.modinfo.json',json.dumps(core,ensure_ascii=False,indent=2).encode('utf8'))
        membership=json.loads(z.read('Integrations/ScCsgoBundle.json'));membership.update(version='1.5.3',core='1.5.3',sourceBundleSha256=old['sha256'],coreSha256=sha(dll))
        membership['sourcePackagesNote']='Historical resource provenance; gameplay core replaced by the tested 1.5.3 DLL.'
        add('Integrations/ScCsgoBundle.json',json.dumps(membership,ensure_ascii=False,indent=2).encode('utf8'))
    add('ScCsgoKnives.dll',dll)
    add('INSTALL.txt',('''CS武器1.5.3单文件总包，直接导入scmod。全量与轻量二选一。
...
正在充能的旧枪保留剩余秒数及已保存周期。格式明确的局部缺记录/型号冲突不再阻止整个世界：先备份，正常枪可用，异常枪原样保留待恢复；所有旧引用编号避让分配，不以新枪默认状态恢复。
完整记录表丢失、未知格式或备份失败仍会拒绝进入。旧World7的M249/M4A1-S原状态仍需丢失前的备份，不能凭空补回。
CT/T玩家外观仍需另装NekoMeko Model1.1及Neorxna1.4。不要同时装旧战术拓展。
离线测试不等于完整实机/Android验收；还原旧版时同时恢复升级前完整世界备份。
''').encode('utf-8-sig'))
    target=OUT/f'[API1.9]CS武器1.5.3-作者ZH667-{"512轻量" if a.edition=="Lite" else "全量"}总包.scmod'
    ...
    reports=OUT/'release-single-1.5.3';reports.mkdir(exist_ok=True)
