"""Stage/publish the full-only 1.3.0 crowd diagnostic. Change one DLL only."""
import argparse
import hashlib
import json
import shutil
...
STAGE = ROOT / '.tmp/crowd-perf-20260926'
NAME = '[API1.9]CS武器1.3.0-全量包.scmod'
SOURCE_SHA = 'b4d31f6e785cb8fb5814c01b9d228dbacf3b097da98c73528ee868eaf9a38099'
...
def stage():
    ...
    with zipfile.ZipFile(source) as old:
        ...
        with zipfile.ZipFile(target) as new:
            ...
            report = dict(path=NAME, revision='crowd-diag-20260926', sourceSha256=SOURCE_SHA,
...
def publish():
    ...
    for tool in ['TacticalPerformanceCheck','AppearanceCheck','TacticalRenderCheck']:
    ...
    report.update(validation=validation,probe=probe,appearanceChecks=len(appearance['checks']),nativeActorFrames=len(actors),
        scope='Windows isolated native fixtures; no Android, actual player world, or all-installed-mod combination acceptance')
    (ROOT/'docs/release-crowd-diagnostics-1.3.0-2026-09-26-evidence.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n','utf-8')
