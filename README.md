...
《生存战争 2》SCAPI 1.9.2.1 的 CS2 武器模组，与 `ScCsgoBox` 联动。当前开发版：0.26.2（生存内容第二轮完善，包内程序检查通过，实机验收待完成）。
...
- 投掷物按住左键准备强投，按住右键或“轻投”按钮准备近抛，松开才出手；准备时可持续瞄准，切走可取消，出手后才扣除并开始引信计时。高爆、闪光、烟雾、燃烧瓶、燃烧弹和诱饵均有制作配方。
...
安装包：`output/ScCsgoKnives-0.26.2.scmod`。资源来源见 `ASSET_SOURCES.md`，第三方署名见 `THIRD_PARTY_NOTICES.md`。
