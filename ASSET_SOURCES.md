...
The supply atlas is encoded as RGBA with alpha 255; all decoded color pixels and its 256×128 size are identical to 0.27.0 in that release. The authored 2026-09-24 source revision below supersedes this atlas. Original weapon texture quality is unchanged.

### 2026-09-24 authored supply models (source revision)

`ScSupplyGeometry.cs` authors eight supply meshes and five radio visual variants. `tools/build_supply_surface.py` generates their shared opaque 512×256 atlas with 25 padded material/marking cells using deterministic drawing primitives. Geometry, icons and marks are authored here; no TaCZ models or artwork are imported. Active radios now reuse the core procedural mesh/atlas instead of the historical radio GLB; the old GLB/PNG remain preserved. CS2-imported repair kits, defusers and weapon assets are unchanged. See `docs/supply-models-2026-09-24.md` and its evidence JSON for source hashes, offline views and validation limits. No release package is claimed by this source revision.
...
Model, skinned hands animation and audio extracted from the locally installed CS2 `pak01_dir.vpk`. Evidence: `docs/c4-source-109.json`. Source plant clip is 4 seconds; item commits at 3.2 seconds, with a 20-second gameplay fuse. Beep cadence and Survivalcraft blast effects are adaptations; no claim of identical CS2 explosion simulation. Full retains source texture dimensions; Optimized512 derives Q85 WebP and reduced geometry.

## Knife finishes (1.0.13)

Knife meshes and the finish sources were read from the installed CS2 `pak01_dir.vpk`. The inventory images in the package are the decoded official `default_generated/*_light_png` renders; they are not recolored copies. The ordinary Bayonet is fixed to CS2 paint kit 415, `am_ruby_marbleized` (Ruby). Other knife variants use a mix of verified Gamma Doppler phases and Fade, and factory variants remain alongside the finish variants. The two default CT/T models have no fabricated finish.

The shipped body maps are a local metallic material port onto the CS2 knife UVs. They use the official paint pattern, masks, recipe colors and roughness where available. In 1.0.14, a 0.86 RGB multiplier is applied to the coat before mask compositing in response to the user's report of pale colors; uncoated substrate pixels are unchanged. This is a visual adjustment, not evidence that lighting caused the issue or a measured match to CS2. It does not claim pixel-identical Source 2 compositor output, wear seed, pearlescence or in-game GPU lighting. Source hashes and pairings are recorded in `docs/knife-finishes-source-20260913.json`; rejected earlier tinted previews are listed in `docs/retired-knife-preview-assets-20260913.json`.

## Casing effects (1.2.1)
