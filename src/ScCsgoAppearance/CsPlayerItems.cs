...
public static class CsPlayerItems {
    public static void Draw(ComponentHumanModel human, Camera camera, int value) {
        ...
        var world = weapon?.HasRightGrip == true
            ? ScThirdPersonMath.WeaponWorld(weapon.GripRight, handWorld.Translation, human.m_componentCreature.ComponentBody.Matrix.Forward, Vector3.UnitY)
        ...
        foreach (var group in weapon.Groups) {
            ...
            if (native) ScGunNativeMesh.DrawWorld(renderer, group.Mesh, texture, Color.White, 1, ref view, env);
            else BlocksManager.DrawMeshBlock(renderer, group.Mesh, texture, Color.White, 1, ref view, env);
        }
