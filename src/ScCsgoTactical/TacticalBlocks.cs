...
public sealed class ScTacticalBeaconBlock : Block {
    ...
    public override void Initialize(){base.Initialize();TacticalItemMesh.LoadRadios();TacticalItemMesh.Load("repair_item");}
...
public sealed class ScTacticalSquadBlock : Block {
    ...
    public override void Initialize(){base.Initialize();TacticalItemMesh.LoadRadios();}
    ...
    public override void GenerateTerrainVertices(BlockGeometryGenerator g,TerrainGeometry t,int v,int x,int y,int z){}
    public override void DrawBlock(PrimitivesRenderer3D r,int value,Color color,float size,ref Matrix matrix,DrawBlockEnvironmentData env)=>TacticalItemMesh.DrawRadio(TacticalItemMesh.SquadRadioKind(Terrain.ExtractData(value)),r,color,size,ref matrix,env);
}
