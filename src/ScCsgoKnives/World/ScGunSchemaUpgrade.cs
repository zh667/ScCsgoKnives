...
/// <summary>Historical snapshot utilities plus manual-backup migration entry points.
///
/// Loading a schema 1 through 5 table converts it in memory and the next normal save writes schema 6, which older
...
///
/// User policy 2026-09-25 supersedes the automatic behavior described above: BeforeLoad and
/// BeforeReleaseLoad never create files. Snapshot/Mark are retained for explicit tooling and historical tests;
/// no gameplay path calls them. Existing backup provenance is not deleted.
///
...
public static class ScGunSchemaUpgrade {
    ...
    /// <summary>Called after the format guard. Records the conversion in logs; no disk access.</summary>
    public static string BeforeLoad(XElement project, WorldInfo world) {
        if (NeedsBackup(project)) KnifeLog.Information($"gun schema {SavedSchema(project)} -> {ScGunRegistry.Schema}: backups=manual; validated conversion retains existing gun state");
        return null;
    }

    /// <summary>Historical ABI retained; users now manage all restore points themselves.</summary>
    public static string BeforeReleaseLoad(XElement project, WorldInfo world, string verifiedBackup = null) {
        // Retained entry point for compatibility revisions. Never creates an automatic restore point.
        return null;
    }
}
