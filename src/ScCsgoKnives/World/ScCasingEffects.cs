...
public sealed class ScCasingEffects {
    ...
    public sealed class Debris {public Vector3 Position,Velocity,Spin;public float Age;public int Bounces;public string Model;}
    ...
    public void Update(float dt,SubsystemTerrain terrain,SubsystemAudio audio){
        ...
        for(int i=m_live.Count-1;i>=0;i--){
            ...
            if(d.Bounces>=(ScResourcePolicy.Lite?1:2))continue;
            ...
            if(hit.HasValue){Vector3 n=CellFace.FaceToVector3(hit.Value.CellFace.Face);d.Position=hit.Value.HitPoint()+n*.015f;d.Velocity=(d.Velocity-2*Vector3.Dot(d.Velocity,n)*n)*.32f;d.Bounces++;
                if(now-m_lastImpact>.1){m_lastImpact=now;audio.PlaySound("Audio/ScCsgoKnives/casing_"+(d.Model is "nova_casing" or "mag7_casing" or "xm1014_casing"?"shotgun":"metal"),.18f,0,d.Position,3,false);}
    ...
    public void Draw(Camera camera){
        foreach(var d in m_live){if(Vector3.DistanceSquared(d.Position,camera.ViewPosition)>24*24 || m_failed.Contains(d.Model))continue;
            if(!m_models.TryGetValue(d.Model,out var model)){
                try {
                var info=Definitions.Models[d.Model];var source=ContentManager.Get<Model>("Models/ScCsgoKnives/"+info.Model);var mesh=new BlockMesh();
            ...
            Matrix world=Matrix.CreateFromYawPitchRoll(d.Spin.Y*d.Age,d.Spin.X*d.Age,d.Spin.Z*d.Age)*Matrix.CreateTranslation(d.Position);
