...
public sealed class ScGunBindingsScreen : Screen {
    ...
    readonly LabelWidget m_title = ScGunUi.Label("CS 武器 · 键盘与手柄绑定");
    ...
    readonly Dictionary<string, ButtonWidget> m_padButtons = [];
    Dictionary<string, string> m_padWorking;
    readonly SliderWidget m_threshold = ScGunUi.Slider(.15f,.9f,.05f,.5f,"手柄扳机阈值");
    ...
    public ScGunBindingsScreen() {
        ...
        foreach (var id in ScGunFunctions.All) {
            var b = ScGunUi.Button("",280); b.Margin = new Vector2(0,4);
            ((BevelledButtonWidget)b).m_labelWidget.WordWrap = true;
            m_padButtons[id] = b; m_list.Children.Add(b);
        }
    ...
    public override void Enter(object[] parameters) {
        ...
        m_padWorking = ScGunFunctions.All.ToDictionary(id=>id,ScGamepadBindings.Get);
        m_threshold.Value = ScGamepadBindings.Threshold;
        ScWeaponTouchPanel.SuppressAll(true); m_status.Text = ""; Refresh();
    }
    void Refresh() {
        foreach (var (id,b) in m_buttons) b.Text = ScGunBindings.BindingSummary(id,m_working[id]);
        foreach (var (id,b) in m_padButtons) b.Text = ScGunBindings.Label(id)+"："+ScGamepadBindings.Label(m_padWorking[id]);
    }
    public override void MeasureOverride(Vector2 available) {
        ...
        foreach(var b in m_buttons.Values.Concat(m_padButtons.Values)) { ((CanvasWidget)b).Size = new Vector2(Math.Min(540,w-48),64);((BevelledButtonWidget)b).FontScale=.85f; }
    ...
    public override void Update() {
        foreach (var (id,b) in m_padButtons) if (b.IsClicked) {
            DialogsManager.ShowDialog(this,new ListSelectionDialog("手柄："+ScGunBindings.Label(id),ScGamepadBindings.Options(),52,
                x=>ScGamepadBindings.Label((string)x),x=> {
                    string key=(string)x;
        ...
        if(m_reset.IsClicked){foreach(var id in ScGunFunctions.All){m_working[id]=ScGunBindings.Default(id);m_padWorking[id]=ScGamepadBindings.Default(id);}m_threshold.Value=.5f;Refresh();m_status.Text="尚未保存";}
        ...
        if(m_save.IsClicked) {
            ...
            var padBefore=new Dictionary<string,string>(ScGamepadBindings.Keys); float thresholdBefore=ScGamepadBindings.Threshold;
            ScGamepadBindings.Keys.Clear(); foreach(var p in m_padWorking)ScGamepadBindings.Keys[p.Key]=p.Value;
            ScGamepadBindings.Threshold=m_threshold.Value;
            ...
            ScGamepadBindings.Keys.Clear();foreach(var p in padBefore)ScGamepadBindings.Keys[p.Key]=p.Value;
            ScGamepadBindings.Threshold=thresholdBefore;
