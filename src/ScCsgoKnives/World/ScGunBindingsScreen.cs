...
public sealed class ScGunBindingsScreen : Screen {
    ...
    public ScGunBindingsScreen() {
        ...
        m_list.Children.Add(ScGunUi.Note("点击功能选择按键或解除绑定。若选择移动、背包等原版已用键，原版功能也可能触发，请在映射器和游戏设置中避开冲突。"));
        foreach (var id in ScGunFunctions.All) { var b = ScGunUi.Button("", 280); b.Margin = new Vector2(0, 4); m_buttons[id] = b; m_list.Children.Add(b); }
    ...
    void Refresh() { foreach (var (id,b) in m_buttons) b.Text = ScGunBindings.Label(id) + "：" + (m_working[id].Length > 0 ? m_working[id] : "未绑定"); }
    public override void MeasureOverride(Vector2 available) {
        ...
        foreach(var b in m_buttons.Values) ((CanvasWidget)b).Size = new Vector2(Math.Min(420,w-48),48);
    ...
    public override void Update() {
        foreach (var (id,b) in m_buttons) if(b.IsClicked) {
            ...
            DialogsManager.ShowDialog(this,new ListSelectionDialog("绑定："+ScGunBindings.Label(id),options,52,
                x=>(string)x==""?"解除绑定":(string)x,x=> {
                    string key=(string)x;
