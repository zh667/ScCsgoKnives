...
/// <summary>Ghoul 2.0 copies whole player XML but not our world registry. Transport only records
...
public static class ScGunTravel {
    ...
    static bool Player(XElement e)=>(string)e.Attribute("Name") is "MalePlayer" or "FemalePlayer";
    ...
    static IEnumerable<(XElement Field,int Id,int Variant)> Items(XElement entity,int block) {
        foreach(var inventory in entity.Elements("Values")) {
            ...
            foreach(var slot in slots.Elements("Values")) {
                if(!int.TryParse(Text(slot,"Contents"),out int value)||!int.TryParse(Text(slot,"Count"),out int count)||count<=0)continue;
    ...
    public static void Capture(XElement project,string world) {
        ...
        var registry=Group(gun,"GunRegistry");if(Text(registry,"Schema")!="3")return;
        var rows=Map(Group(registry,"Records"));var identities=Map(Group(gun,Identities));int block=Index(project);
        if(block<0)return;
        foreach(string id in rows.Keys)if(int.TryParse(id,out int number)&&!identities.ContainsKey(id))identities[id]=Token(world,number);
        ...
        foreach(var player in project.Element("Entities")?.Elements().Where(Player)??[]) {
            var packet=Make(Packet);packet.Add(Field("Version",1),Field("Source",world),Field("GrowthMode",Text(registry,"GrowthMode")??"Unset"));
            ...
            try {
                ...
                var live=ScGunRegistry.Current;
                if(carried.HasElements && (live?.Kills.Count>0 || live?.Recovery.Count>0))throw new InvalidOperationException("有待结算击杀或物品补偿，请结算后再传送");
    ...
    public static Plan Prepare(XElement source,string target) {
        ...
        var incoming=players.Where(p=>Group(p,Packet) is {} packet && Text(packet,"Source")!=target).ToArray();
        ...
        var outside=new XElement(doc);foreach(var p in outside.Element("Entities")?.Elements().Where(Player).Where(p=>Group(p,Packet)!=null&&Text(Group(p,Packet),"Source")!=target).ToArray()??[])p.Remove();
        ...
        foreach(var player in incoming) {
            ...
            if(Text(packet,"Version")!="1"||!Related(origin,target)||Text(packet,"Error") is not null)throw new InvalidOperationException("跨世界携带快照不兼容或未就绪："+Text(packet,"Error"));
            ...
            var proof=new ValuesDictionary();proof.SetValue("Schema",3);proof.SetValue("Next",1023);
            ...
            foreach(var item in Items(player,block)) {
                string old=item.Id.ToString(CultureInfo.InvariantCulture);
                if(!rows.TryGetValue(old,out var row)||!keys.TryGetValue(old,out var identity)||identity.Length!=64||!seen.Add(identity)||!parsed.TryGetSnapshot(item.Id,out var snap)||snap.Variant!=item.Variant)
            ...
            packet.Remove();
            if(count>0&&registry.GrowthMode==ScGunGrowthMode.Unset)saved.SetValue("GrowthMode",Text(packet,"GrowthMode")??"Unset");
            else if(count>0&&Text(packet,"GrowthMode") is {} mode && mode!="Unset"&&mode!=registry.GrowthMode.ToString())throw new InvalidOperationException("两个世界计数成长规则不同，拒绝静默改变枪械加成");
        }
    ...
    public static void BeforeLoad(XElement source,WorldInfo world) {
        ...
        source.ReplaceNodes(plan.Document.Nodes());KnifeLog.Information($"[GUN_TRAVEL] imported {plan.Guns} carried guns; layout 5/schema 3 preserved");
    }
}
