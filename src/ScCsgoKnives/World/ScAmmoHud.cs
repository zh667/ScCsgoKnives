    ...
    public static ScAmmoReadout Read(GunSpec gun, int value, IInventory inventory, bool creative,
        ...
        if (gun.RechargeSeconds > 0) {
            if (rounds > 0) return new(text("Ready"), text("SingleCharge"), false, false, false, wear, wearState) {
                Compact = true, LoadedText = "1", CapacityText = "/ 1", ReserveText = "", Icon = gun.Name + "_slot"
            };
            ...
            return new(Format("Charging", Math.Ceiling(remaining * 10) / 10), text("AutoCharge"), true, true, false, wear, wearState) {
                ...
                ReserveText = "", Status = "充能", Icon = gun.Name + "_slot"
            };
        }
        ...
        return new(main, detail, rounds == 0, false, !creative && !covered && reserve < cost, wear, wearState) {
            ...
            Status = reloading ? "装填" : "", Icon = gun.Name + "_slot"
        };
    }
}

/// <summary>Passive readout above the central hotbar; smaller readable type on mobile.</summary>
public sealed class ScAmmoHud : IDisposable {
    ...
    void Position() {
        ...
        Vector2 size = new(Math.Max(190, Panel.ActualSize.X), Math.Max(44, Panel.ActualSize.Y));
        ...
        Vector2 corner = FindCenter(host.ActualSize, size, obstacles);
        Panel.MarginRight = 0; Panel.MarginLeft = 0;
        Panel.MarginBottom = Math.Max(0, host.ActualSize.Y - corner.Y - size.Y); Panel.MarginTop = 0;
    }
    public ScAmmoHud() {
        Panel.Children.Add(Main); Panel.Children.Add(Detail); Panel.Children.Add(Wear);
